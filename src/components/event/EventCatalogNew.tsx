...
import { Search, MapPin, Clock, SlidersHorizontal, X, Loader2, ArrowRight } from 'lucide-react';
import { apiFetch, buildQuery } from '@/lib/api';
import type { BackendEvent } from '@/types/event';
import { format, parseISO } from 'date-fns';

interface EventCatalogNewProps {
    onEventSelect: (eventId: string) => void;
}

export function EventCatalogNew({ onEventSelect }: EventCatalogNewProps) {
                                    ...
                                    >
                                        <div className="bg-card rounded-3xl overflow-hidden border border-border hover:border-foreground/30 transition-all duration-300 shadow-soft hover:shadow-soft-hover h-full flex flex-col">
                                            ...
                                            <div className="relative aspect-[16/10] overflow-hidden bg-muted">
                                                <img
                                                    src={event.flyer || `https://images.unsplash.com/photo-1540575467063-178a50c2df87?w=600&h=400&fit=crop&seed=${event.id}`}
                                                    alt={event.title}
